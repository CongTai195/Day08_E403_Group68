...
def score_faithfulness(
    ...
    Sử dụng LLM-as-Judge để chấm điểm tự động.
    """
    import json
    from rag_answer import call_llm
    
    context = "\n".join([c.get("text", "")[:500] for c in chunks_used[:3]])
...
Rate from 1-5:
5 = Completely grounded, all information is from context
4 = Almost all grounded, minor uncertainty
3 = Mostly grounded, some info may be from model knowledge  
2 = Many claims not in context
1 = Not grounded, mostly made up

Output ONLY valid JSON: {{"score": <1-5>, "reason": "<brief reason>"}}"""
    ...
    try:
        ...
        return {"score": None, "notes": "LLM scoring failed"}
...
def score_answer_relevance(
    ...
    prompt = f"""You are an evaluation judge. Rate the relevance of an answer to the question.

Question: {query}

Answer: {answer}

Answer Relevance measures if the answer directly addresses the question.
Rate from 1-5:
5 = Directly and completely answers the question
4 = Answers correctly but missing minor details
3 = Related but not focused on the core question
...
def score_completeness(
    ...
    prompt = f"""You are an evaluation judge. Rate the completeness of an answer.
...
Completeness measures if the answer covers all key points from the expected answer.
Rate from 1-5:
5 = Covers all important points from expected answer
...
if __name__ == "__main__":
    ...
    print(f"\nBaseline scorecard saved: {scorecard_path}")

    # --- Chạy Variant ---
    print("\n" + "="*60)
    print("VARIANT EVALUATION (Hybrid + Rerank)")
    print("="*60)
    ...
    print(f"\nVariant scorecard saved: {variant_path}")
